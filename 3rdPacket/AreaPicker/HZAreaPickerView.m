...
- (id)initWithStyle:(HZAreaPickerStyle)pickerStyle withDelegate:(id <HZAreaPickerDelegate>)delegate andDatasource:(id <HZAreaPickerDatasource>)datasource
{
    ...
    if (self) {
        ...
        cities = [[provinces objectAtIndex:0] objectForKey:@"sub"];//cities
        ...
        if (self.pickerStyle == HZAreaPickerWithStateAndCityAndDistrict) {
            ...
            areas = [[cities objectAtIndex:0] objectForKey:@"sub"];//areas
            if (areas.count > 0) {
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    if (self.pickerStyle == HZAreaPickerWithStateAndCityAndDistrict) {
        switch (component) {
            case 0:
                cities = [[provinces objectAtIndex:row] objectForKey:@"sub"];//cities
                ...
                break;
            case 1:
                areas = [[cities objectAtIndex:row] objectForKey:@"sub"];//areas
                [self.locatePicker selectRow:0 inComponent:2 animated:YES];
                [self.locatePicker reloadComponent:2];
